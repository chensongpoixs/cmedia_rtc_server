...
#define D3DFVF_CUSTOMVERTEX (D3DFVF_XYZ | D3DFVF_TEX1)
	...
	VideoRenderer* VideoRenderer::CreatePlatformRenderer(const char* window_title,
		...
		return D3dRenderer::Create(window_title, width, height, track_to_render);
	}

	D3dRenderer::D3dRenderer(size_t width, size_t height, webrtc::VideoTrackInterface* track_to_render)
		: width_(width),
		height_(height),
		hwnd_(nullptr),
		d3d_(nullptr),
	...
	D3dRenderer* D3dRenderer::Create(const char* window_title,
		...
		D3dRenderer* d3d_renderer = new D3dRenderer(width, height, track_to_render);
	...
	void D3dRenderer::OnFrame(const webrtc::VideoFrame& frame) {
		...
		d3d_device_->SetStreamSource(0, vertex_buffer_.release(), 0, sizeof(D3dCustomVertex));
		d3d_device_->SetTexture(0, texture_.release());
