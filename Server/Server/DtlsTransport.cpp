...
#define LOG_OPENSSL_ERROR(desc)                                                                    \
	...
	{                                                                                                \
		if (ERR_peek_error() == 0)                                                                     \
		{ ERROR_EX_LOG("OpenSSL error [desc:'%s']", desc);    }                                             \
		else                                                                                           \
		{                                                                                              \
			int64_t err;                                                                                 \
			while ((err = ERR_get_error()) != 0)                                                         \
			{                                                                                            \
				ERROR_EX_LOG("OpenSSL error [desc:'%s', error:'%s']", desc, ERR_error_string(err, nullptr));   \
			}                                                                                            \
			ERR_clear_error();                                                                           \
		}                                                                                              \
	} while (false)
...
namespace RTC
{
	...
	{
		{ chen::ECRYPTO_AEAD_AES_256_GCM, "SRTP_AEAD_AES_256_GCM" },
		{ chen::ECRYPTO_AEAD_AES_128_GCM, "SRTP_AEAD_AES_128_GCM" },
		{ chen::ECRYPTO_AES_CM_128_HMAC_SHA1_80, "SRTP_AES128_CM_SHA1_80" },
		{ chen::ECRYPTO_AES_CM_128_HMAC_SHA1_32, "SRTP_AES128_CM_SHA1_32" }
	};
	// clang-format on
	...
	void DtlsTransport::GenerateCertificateAndPrivateKey()
	{
		...
		/*X509_NAME*/X509_name_st* certName = NULL;
		std::string subject =
		  std::string("media_rtc_server") + std::to_string(s_crypto_random. GetRandomUInt(100000, 999999));
		...
		ASN1_INTEGER_set(
		  ...
		  static_cast<uint64_t>(s_crypto_random. GetRandomUInt(1000000, 9999999)));
	...
	void DtlsTransport::CreateSslCtx()
	{
		...
		DEBUG_EX_LOG("dtls, srtp, setting SRTP cryptoSuites for DTLS: %s", dtlsSrtpCryptoSuites.c_str());
	...
	void DtlsTransport::GenerateFingerprints()
	{
		...
		for (auto& kv : DtlsTransport::string2FingerprintAlgorithm)
		{
			...
			DEBUG_EX_LOG("dtls, %-7s fingerprint: %s", algorithmString.c_str(), hexFingerprint);
	...
	void DtlsTransport::Dump() const
	{
		...
		DEBUG_LOG("<DtlsTransport>");
		DEBUG_LOG("  state           : %s", state.c_str());
		DEBUG_LOG("  role            : %s", role.c_str());
		DEBUG_LOG("  handshake done: : %s", this->handshakeDone ? "yes" : "no");
		DEBUG_LOG("</DtlsTransport>");
	}
	...
	void DtlsTransport::Run(Role localRole)
	{
		...
		{
			ERROR_LOG("same local DTLS role provided, doing nothing");
		...
		{
			DEBUG_EX_LOG("dtls, resetting DTLS due to local role change");
		...
		switch (this->localRole)
		{
			case Role::CLIENT:
			{
				DEBUG_EX_LOG("dtls, running [role:client]");
			...
			case Role::SERVER:
			{
				DEBUG_EX_LOG("dtls, running [role:server]");
	...
	//	MS_ASSERT(
		...
		{
			DEBUG_EX_LOG("dtls, handshake already done, processing it right now");
	...
	void DtlsTransport::ProcessDtlsData(const uint8_t* data, size_t len)
	{
		...
		if (!IsRunning())
		{
			ERROR_LOG("cannot process data while not running");
		...
		{
			WARNING_EX_LOG("dtls, OpenSSL BIO_write() wrote less (%zu bytes) than given data (%zu bytes)",
		...
		if (read > 0)
		{
			...
			{
				WARNING_EX_LOG("dtls, ignoring application data received while DTLS handshake not done");
	...
	void DtlsTransport::SendApplicationData(const uint8_t* data, size_t len)
	{
		...
		{
			WARNING_EX_LOG("dtls, cannot send application data while DTLS is not fully connected");
		...
		if (len == 0)
		{
			WARNING_EX_LOG("dtls, ignoring 0 length data");
		...
		{
			WARNING_EX_LOG( "dtls, OpenSSL SSL_write() wrote less (%d bytes) than given data (%zu bytes)", written, len);
		}
	...
	void DtlsTransport::Reset()
	{
		...
		WARNING_EX_LOG("dtls, resetting DTLS transport");
	...
	inline bool DtlsTransport::CheckStatus(int returnCode)
	{
		...
		switch (err)
		{
			...
			case SSL_ERROR_WANT_WRITE:
				WARNING_EX_LOG("dtls, SSL status: SSL_ERROR_WANT_WRITE");
				break;

			case SSL_ERROR_WANT_X509_LOOKUP:
				WARNING_EX_LOG("dtls, SSL status: SSL_ERROR_WANT_X509_LOOKUP");
				break;
			...
			case SSL_ERROR_WANT_CONNECT:
				WARNING_EX_LOG("dtls, SSL status: SSL_ERROR_WANT_CONNECT");
				break;

			case SSL_ERROR_WANT_ACCEPT:
				WARNING_EX_LOG("dtls, SSL status: SSL_ERROR_WANT_ACCEPT");
				break;

			default:
				WARNING_EX_LOG("dtls, SSL status: unknown error");
		}
		...
		else if (((SSL_get_shutdown(this->ssl) & SSL_RECEIVED_SHUTDOWN) != 0) || err == SSL_ERROR_SSL || err == SSL_ERROR_SYSCALL)
		{
			if (this->state == DtlsState::CONNECTED)
			{
				DEBUG_EX_LOG("dtls, disconnected");
			...
			{
				WARNING_EX_LOG("dtls, connection failed");
	...
	inline void DtlsTransport::SendPendingOutgoingDtlsData()
	{
		...
		DEBUG_EX_LOG("%llu bytes of DTLS data ready to sent to the peer", read);
	...
	inline bool DtlsTransport::SetTimeout()
	{
		...
		{
			DEBUG_EX_LOG("DTLS timer set in %llu ms", timeoutMs);
		...
		{
			WARNING_EX_LOG("dtls, DTLS timeout too high (%llu ms), resetting DLTS", timeoutMs);
	...
	inline bool DtlsTransport::ProcessHandshake()
	{
		...
		/*RTC::SrtpSession::CryptoSuite*/  chen::ECRYPTO_SUITE srtpCryptoSuite = GetNegotiatedSrtpCryptoSuite();

		if (srtpCryptoSuite != chen::ECRYPTO_NONE/*RTC::SrtpSession::CryptoSuite::NONE*/)
		{
	...
	inline bool DtlsTransport::CheckRemoteFingerprint()
	{
		...
		if (!certificate)
		{
			WARNING_EX_LOG("dtls, no certificate was provided by the peer");
		...
		{
			WARNING_EX_LOG("dtls, fingerprint in the remote certificate (%s) does not match the announced one (%s)",
			  hexFingerprint,
		...
		DEBUG_EX_LOG("dtls, valid remote fingerprint");
	...
	inline void DtlsTransport::ExtractSrtpKeys(chen::ECRYPTO_SUITE /*RTC::SrtpSession::CryptoSuite*/ srtpCryptoSuite)
	{
		//MS_TRACE();
		...
		{
		case chen::ECRYPTO_AES_CM_128_HMAC_SHA1_80/*RTC::SrtpSession::CryptoSuite::AES_CM_128_HMAC_SHA1_80*/:
		case  chen::ECRYPTO_AES_CM_128_HMAC_SHA1_32 /*RTC::SrtpSession::CryptoSuite::AES_CM_128_HMAC_SHA1_32*/:
			{
		...
		case chen::ECRYPTO_AEAD_AES_256_GCM/*RTC::SrtpSession::CryptoSuite::AEAD_AES_256_GCM*/:
			{
		...
		case chen::ECRYPTO_AEAD_AES_128_GCM/*RTC::SrtpSession::CryptoSuite::AEAD_AES_128_GCM*/:
			{
	...
	inline chen::ECRYPTO_SUITE /*RTC::SrtpSession::CryptoSuite*/ DtlsTransport::GetNegotiatedSrtpCryptoSuite()
	{
		//MS_TRACE();

		/*RTC::SrtpSession::CryptoSuite*/chen::ECRYPTO_SUITE negotiatedSrtpCryptoSuite = chen::ECRYPTO_NONE/*RTC::SrtpSession::CryptoSuite::NONE*/;
		...
		for (auto& srtpCryptoSuite : DtlsTransport::srtpCryptoSuites)
		{
			...
			{
				DEBUG_EX_LOG("dtls, srtp, chosen SRTP crypto suite: %s", cryptoSuiteEntry->name);
	...
	inline void DtlsTransport::OnSslInfo(int where, int ret)
	{
		...
		{
			DEBUG_EX_LOG("dtls, [role:%s, action:'%s']", role, SSL_state_string_long(this->ssl));
		}
		else if ((where & SSL_CB_ALERT) != 0)
		{
			...
			{
				WARNING_EX_LOG("dtls, received DTLS %s alert: %s", alertType, SSL_alert_desc_string_long(ret));
			}
			else if ((where & SSL_CB_WRITE) != 0)
			{
				DEBUG_EX_LOG("dtls, sending DTLS %s alert: %s", alertType, SSL_alert_desc_string_long(ret));
			}
			else
			{
				DEBUG_EX_LOG("dtls, DTLS %s alert: %s", alertType, SSL_alert_desc_string_long(ret));
			}
		}
		else if ((where & SSL_CB_EXIT) != 0)
		{
			if (ret == 0)
				DEBUG_EX_LOG("dtls, [role:%s, failed:'%s']", role, SSL_state_string_long(this->ssl));
			else if (ret < 0)
				DEBUG_EX_LOG("dtls, role: %s, waiting:'%s']", role, SSL_state_string_long(this->ssl));
		}
		else if ((where & SSL_CB_HANDSHAKE_START) != 0)
		{
			DEBUG_EX_LOG("dtls, DTLS handshake start");
		}
		else if ((where & SSL_CB_HANDSHAKE_DONE) != 0)
		{
			DEBUG_EX_LOG("dtls, DTLS handshake done");
	...
	inline void DtlsTransport::OnTimer(/*Timer**/ /*timer*/)
	{
		...
		if (this->handshakeDone)
		{
			DEBUG_EX_LOG("handshake is done so return");
