...
#include "crtp.h"
namespace chen {
	ch264_source * ch264_source::construct()
	{
		return (new ch264_source());
	}
	void ch264_source::destroy(ch264_source * source)
	{
		delete source;
	}
	bool ch264_source::init(uint32 frame_rate)
	{
		m_frame_rate = frame_rate;
		return true;
	}
	void ch264_source::destroy()
	{
	}
	std::string ch264_source::get_media_description(uint16 port)
	{
		char buf[100] = { 0 };
		sprintf(buf, "m=video %hu RTP/AVP 96", port); // \r\nb=AS:2000
		return std::string(buf);
	}
	std::string ch264_source::get_attribute()
	{
		return std::string("a=rtpmap:96 H264/90000");
	}
	bool ch264_source::handler_frame(MediaChannelId channel_id, AVFrame frame)
	{
		...
		if (frame_size <= MAX_RTP_PAYLOAD_SIZE) {
			...
			{
				if (!m_send_frame_callback_ptr(channel_id, rtp_pkt)) 
				{
					return false;
				}
			}
