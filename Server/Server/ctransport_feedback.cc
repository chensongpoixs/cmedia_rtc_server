...
static const  size_t kTransportFeedbackHeaderSizeBytes = 4 + 8 + 8;
static const size_t kChunkSizeBytes = 2;
...
static const size_t kMaxSizeBytes = (1 << 16) * 4;
// Payload size:
// * 8 bytes Common Packet Format for RTCP Feedback Messages
// * 8 bytes FeedbackPacket header.
// * 2 bytes for one chunk.
static const size_t kMinPayloadSizeBytes = 8 + 8 + 2;
static const int kBaseScaleFactor = TransportFeedback::kDeltaScaleFactor * (1 << 8);
...
*/
 ...
 const uint8_t TransportFeedback::kFeedbackMessageType;
 const size_t TransportFeedback::kMaxReportedPackets;

 const size_t TransportFeedback::LastChunk::kMaxRunLengthCapacity;
 const size_t TransportFeedback::LastChunk::kMaxOneBitCapacity;
 const size_t TransportFeedback::LastChunk::kMaxTwoBitCapacity;
   const size_t TransportFeedback::LastChunk::kMaxVectorCapacity;
...
{
  cassert(!CanAdd(0) || !CanAdd(1) || !CanAdd(2));
  if (all_same_) 
  {
  ...
  cassert(size_>= kMaxTwoBitCapacity);
...
{
  cassert(!has_large_delta_);
  cassert(size_<= kMaxOneBitCapacity);
...
bool TransportFeedback::AddReceivedPacket(uint16_t sequence_number, int64_t timestamp_us) 
{
  ...
  if (include_timestamps_) 
  {
    ...
    if (delta != delta_full) 
	{
      WARNING_EX_LOG("Delta value too large ( >= 2^16 ticks )");
      return false;
    }
  }
  ...
  if (sequence_number != next_seq_no) 
  {
    ...
    if (!webrtc::IsNewerSequenceNumber(sequence_number, last_seq_no)) 
	{
      return false;
    }
...
bool TransportFeedback::Parse(const CommonHeader& packet) 
{
 ...
 // TRACE_EVENT0("webrtc", "TransportFeedback::Parse");
  ...
  {
    WARNING_EX_LOG( "Buffer too small (%u  bytes) to fit a  FeedbackPacket. Minimum size = %u " , packet.payload_size_bytes(), kMinPayloadSizeBytes);
    return false;
  }
  // 2. 得到payload的位置
  ...
  if (status_count == 0) 
  {
	  WARNING_EX_LOG( "Empty feedback messages not allowed.");
    return false;
  }
  ...
  while (delta_sizes.size() < status_count) 
  {
	...
	{
		WARNING_EX_LOG( "Buffer overflow while parsing packet.");
      Clear();
      return false;
    }
  ...
  if (end_index >= index + recv_delta_size) 
  {
    for (size_t delta_size : delta_sizes) 
	{
      ...
      switch (delta_size) 
	  {
        ...
        case 3:
          Clear();
		  WARNING_EX_LOG( "Invalid delta_size for seq_no = %u", seq_no);
  ...
  //得到真实的字节数
  size_bytes_ = crtcp_packet::kHeaderLength + index;
...
bool TransportFeedback::IsConsistent() const {
  ...
  if (num_seq_no_ != delta_sizes.size()) {
    ERROR_EX_LOG("%u,  packets encoded. Expected %u",  delta_sizes.size(), num_seq_no_);
    return false;
  }
  ...
  for (DeltaSize delta_size : delta_sizes) {
    if (delta_size > 0) {
      if (packet_it == packets_.end()) {
        ERROR_EX_LOG(" Failed to find delta for seq_no = %u " , seq_no);
        return false;
      }
      if (packet_it->sequence_number() != seq_no) {
		  ERROR_EX_LOG( "Expected to find delta for seq_no = %u . Next delta is for = %u",  seq_no, packet_it->sequence_number());
        return false;
      }
      if (delta_size == 1 &&
          (packet_it->delta_ticks() < 0 || packet_it->delta_ticks() > 0xff)) {
		  ERROR_EX_LOG( "Delta = %u   for seq_no  = %u doesn't fit into one byte", packet_it->delta_ticks(), seq_no);
        return false;
      }
  ...
  if (packet_it != packets_.end()) {
    ERROR_EX_LOG( "Unencoded delta for seq_no = %u", packet_it->sequence_number());
    return false;
  }
  if (timestamp_us != last_timestamp_us_) {
     ERROR_EX_LOG( "Last timestamp mismatch. Calculated: %u . Saved: %u", timestamp_us, last_timestamp_us_);
    return false;
  }
  if (size_bytes_ != packet_size) {
	  ERROR_EX_LOG( "Rtcp packet size mismatch. Calculated: %u . Saved: %u" , packet_size,  size_bytes_);
    return false;
  }
  return true;
}
...
std::string ToString(const chen::rtcp::TransportFeedback::ReceivedPacket& packet) 
{
  char buffer[1024] = {0};
