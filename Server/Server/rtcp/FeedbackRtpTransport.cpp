...
inline static int32_t parseReferenceTime(uint8_t* buffer)
{
	...
	uint32_t unsignedVal = rtc_byte::get3bytes(buffer, 0);
...
namespace RTC
{
	namespace RTCP
	{
		  ...
		  : FeedbackRtpPacket(commonHeader)
		{
			...
			this->baseSequenceNumber  = rtc_byte::get2bytes(data, 0);
			this->packetStatusCount   = rtc_byte::get2bytes(data, 2);
			this->referenceTime       = parseReferenceTime(data + 4);
			this->feedbackPacketCount = rtc_byte::get1byte(data, 7);
			this->size                = len;
			...
			while (count < this->packetStatusCount && contentLen > offset)
			{
				...
				{
					WARNING_EX_LOG("rtcp, not enough space for chunk");
				...
				if (!chunk)
				{
					WARNING_EX_LOG("rtcp, invalid chunk");
			...
			{
				WARNING_EX_LOG("rtcp, provided packet status count does not match with content");
			...
			while (chunksIt != this->chunks.end() && contentLen > offset)
			{
				...
				{
					WARNING_EX_LOG("rtcp, not enough space for deltas");
			...
			{
				WARNING_EX_LOG("rtcp, received deltas does not match received status count");
		...
		void FeedbackRtpTransportPacket::Dump() const
		{
			...
			static int count = 0;
			//printf("[count = %u]\n", ++count);
			DEBUG_LOG("<FeedbackRtpTransportPacket>");
			DEBUG_LOG("  base sequence         : %hu"  , this->baseSequenceNumber);
			DEBUG_LOG("  packet status count   : %hu"  , this->packetStatusCount);
			DEBUG_LOG("  reference time        : %u"  , this->referenceTime);
			DEBUG_LOG("  feedback packet count : %hhu"  , this->feedbackPacketCount);
			DEBUG_LOG("  size                  : %zu", GetSize());
			...
			{
				DEBUG_LOG("    %hu ms", static_cast<int16_t>(delta / 4));
			}
			DEBUG_LOG("  </Deltas>");

			auto packetResults = GetPacketResults();

			DEBUG_LOG("  <PacketResults>");
			for (auto& packetResult : packetResults)
			{
				if (packetResult.received)
				{
					DEBUG_LOG(
					  "    seq:%hu, received:yes, receivedAtMs:%llu"  ,
					  packetResult.sequenceNumber,
					  packetResult.receivedAtMs);
				}
				else
				{
					DEBUG_LOG("    seq:%hu, received:no", packetResult.sequenceNumber);
				}
			}
			DEBUG_LOG("  </PacketResults>");
		...
		size_t FeedbackRtpTransportPacket::Serialize(uint8_t* buffer)
		{
			...
			rtc_byte::set2bytes(buffer, offset, this->baseSequenceNumber);
			offset += 2;

			// Packet status count.
			rtc_byte::set2bytes(buffer, offset, this->packetStatusCount);
			offset += 2;

			// Reference time.
			rtc_byte::set3bytes(buffer, offset, static_cast<uint32_t>(this->referenceTime));
			offset += 3;

			// Feedback packet count.
			rtc_byte::set1byte(buffer, offset, this->feedbackPacketCount);
			offset += 1;
			...
			for (auto delta : this->deltas)
			{
				...
				{
				rtc_byte::set1byte(buffer, offset, delta);
					offset += 1u;
				}
				else
				{
					rtc_byte::set2bytes(buffer, offset, delta);
					offset += 2u;
				}
			}
		  ...
		  uint16_t sequenceNumber, uint64_t timestamp, size_t maxRtcpPacketLen)
		{
			...
			// Check if there are too many missing packets.
			{
				...
				{
					WARNING_EX_LOG("RTP missing packet number exceeded");
			...
			{
				WARNING_EX_LOG(
				  "RTP packet delta exceeded [latestTimestamp:%llu, timestamp:%llu]",
			...
			// Check whether another chunks and corresponding delta infos could be added.
			{
				...
				{
					WARNING_EX_LOG("maximum packet size exceeded");
		  ...
		  const uint8_t* data, size_t len, uint16_t count)
		{
			...
			if (len < 2u)
			{
				WARNING_EX_LOG("rtcp, not enough space for FeedbackRtpTransportPacket chunk, discarded");

				return nullptr;
			}

			auto bytes        = rtc_byte::get2bytes(data, 0);
			...
			if (chunkType == 0)
			{
				...
				switch (chunk->GetStatus())
				{
					...
					default:
					{
						WARNING_EX_LOG("invalid status for a run length chunk");
		  ...
		  const uint8_t* data, size_t len, std::vector<int16_t>& deltas, size_t& offset)
		{
			...
			else if (this->status == Status::SmallDelta)
			{
				...
				{
					WARNING_EX_LOG("rtcp, not enough space for small deltas");
				...
				{
					auto delta = static_cast<int16_t>(rtc_byte::get1byte(data, offset));
			...
			else if (this->status == Status::LargeDelta)
			{
				...
				{
					WARNING_EX_LOG("rtcp, not enough space for large deltas");
				...
				for (size_t i{ 0 }; i < this->count; ++i)
				{
					auto delta = static_cast<int16_t>(rtc_byte::get2bytes(data, offset));
		...
		void FeedbackRtpTransportPacket::RunLengthChunk::Dump() const
		{
			...
			DEBUG_LOG("  <RunLengthChunk>");
			DEBUG_LOG("    status : %s", FeedbackRtpTransportPacket::status2String[this->status].c_str());
			DEBUG_LOG("    count  : %hu"  , this->count);
			DEBUG_LOG("  </RunLengthChunk>");
		}
		...
		size_t FeedbackRtpTransportPacket::RunLengthChunk::Serialize(uint8_t* buffer)
		{
			...
			rtc_byte::set2bytes(buffer, 0, bytes);
		  ...
		  const uint8_t* data, size_t len, std::vector<int16_t>& deltas, size_t& offset)
		{
			...
			for (auto status : this->statuses)
			{
				...
				else if (status == Status::SmallDelta)
				{
					if (len < 1u)
					{
						WARNING_EX_LOG("rtcp, not enough space for small delta");
				...
				{
					WARNING_EX_LOG("rtcp, invalid status for one bit vector chunk");
		...
		void FeedbackRtpTransportPacket::OneBitVectorChunk::Dump() const
		{
			...
			DEBUG_LOG("  <OneBitVectorChunk>");
			DEBUG_LOG("    %s", out.str().c_str());
			DEBUG_LOG("  </OneBitVectorChunk>");
		}
		  ...
		  const uint8_t* data, size_t len, std::vector<int16_t>& deltas, size_t& offset)
		{
			...
			for (auto status : this->statuses)
			{
				...
				else if (status == Status::LargeDelta)
				{
					if (len < 2u)
					{
						WARNING_EX_LOG("rtcp, not enough space for large delta");
