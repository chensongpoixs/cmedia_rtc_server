...
namespace RTC
{
	namespace RTCP
	{
		...
		public:
			...
			{
				return uint8_t{ rtc_byte::get1byte((uint8_t*)this->header, 4) };
			}
			void SetFractionLost(uint8_t fractionLost)
			{
				rtc_byte::set1byte((uint8_t*)this->header, 4, fractionLost);
			}
			int32_t GetTotalLost() const
			{
				auto value = uint32_t{ rtc_byte::get3bytes((uint8_t*)this->header, 5) };
			...
			void SetTotalLost(int32_t totalLost)
			{
				...
				rtc_byte::set3bytes(reinterpret_cast<uint8_t*>(this->header), 5, value);
			}
