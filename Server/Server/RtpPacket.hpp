...
namespace RTC
{
	...
	public:
		...
		bool ReadAbsSendTime(uint32_t& absSendtime) const
		{
			...
			absSendtime = chen::rtc_byte::get3bytes(extenValue, 0);
		...
		bool UpdateAbsSendTime(uint64_t ms)
		{
			...
			auto absSendTime = chen::rtc_time::TimeMsToAbsSendTime(ms);

			chen::rtc_byte::set3bytes(extenValue, 0, absSendTime);
		...
		bool ReadTransportWideCc01(uint16_t& wideSeqNumber) const
		{
			...
			wideSeqNumber = chen::rtc_byte::get2bytes(extenValue, 0);
		...
		bool UpdateTransportWideCc01(uint16_t wideSeqNumber)
		{
			...
			chen::rtc_byte::set2bytes(extenValue, 0, wideSeqNumber);
		...
		bool ReadSsrcAudioLevel(uint8_t& volume, bool& voice) const
		{
			...
			volume = chen::rtc_byte::get1byte(extenValue, 0);
		...
		bool ReadVideoOrientation(bool& camera, bool& flip, uint16_t& rotation) const
		{
			...
			uint8_t cvoByte       = chen::rtc_byte::get1byte(extenValue, 0);
