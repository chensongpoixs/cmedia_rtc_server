...
namespace chen {
	...
	BOOL CALLBACK EnumChildWindowsProc(HWND hWnd, LPARAM lParam)
	{
		...
		Cnum++;
		::GetWindowText(hWnd, LPSTR(WindowTitle), 100);
	...
	BOOL CALLBACK EnumWindowsCallback(HWND handle, LPARAM lParam)
	{
		...
		processids.push_back(process_id);
	...
	{ 
		...
		//data.process_id = g_cfg.get_uint32(ECI_3DProcessId) ;//::GetCurrentProcessId();
		data.best_handle = 0;
	...
	HWND FindMainWindow(long long id)
	{
		...
		std::sort(processids.begin(), processids.end(), [] (unsigned long v1, unsigned long v2){
			return v1 > v2;
			});
		for (const unsigned long v : processids)
		{
