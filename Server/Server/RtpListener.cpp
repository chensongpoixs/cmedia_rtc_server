	...
	void RtpListener::AddProducer(RTC::Producer* producer)
	{
		...
		for (auto& encoding : rtpParameters.encodings)
		{
			...
			if (ssrc != 0u)
			{
				...
				else
				{
					...
					ERROR_EX_LOG("ssrc already exists in RTP listener [ssrc:%u]", ssrc);
				}
			}
			...
			if (ssrc != 0u)
			{
				...
				else
				{
					...
					ERROR_EX_LOG("RTX ssrc already exists in RTP listener [ssrc:%u]", ssrc);
				}
			}
		}
		...
		if (!rtpParameters.mid.empty())
		{
			...
			else
			{
				...
				ERROR_EX_LOG("MID already exists in RTP listener [mid:%s]", mid.c_str());
			}
		}
		...
		for (auto& encoding : rtpParameters.encodings)
		{
			...
			else if (rtpParameters.mid.empty())
			{
				...
				ERROR_EX_LOG("RID already exists in RTP listener and no MID is given [rid:%s]", rid.c_str());
			}
		}
	}
