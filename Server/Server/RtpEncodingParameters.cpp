...
namespace RTC
{
	...
	bool RtpEncodingParameters::parse(Json::Value& data)
	{
		//MS_TRACE();
		...
		/*if (!data.isMember("ssrc"))
		{
			WARNING_EX_LOG("encodeings -> not find ssrc failed !!!");
			return false;
		}*/
...
*/
		...
		{
			RtpRtxParameters rtp_rtx;
			rtp_rtx.parse(data["rtx"]);
			this->rtx = rtp_rtx;// RtpRtxParameters(data["rtx"]);
			this->hasRtx = true;
		}
