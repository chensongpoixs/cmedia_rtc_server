...
DelayBasedBwe::Result DelayBasedBwe::IncomingPacketFeedbackVector(
  ...
  if (packet_feedback_vector.empty()) {
   WARNING_EX_LOG("very late feedback received");
...
DelayBasedBwe::Result DelayBasedBwe::MaybeUpdateEstimate(
  ...
  if (delay_detector_->State() == BandwidthUsage::kBwOverusing) {
    DEBUG_EX_LOG("delay_detector_->State() == BandwidthUsage::kBwOverusing");
	DEBUG_EX_LOG("in_alr: %s", in_alr ? "true" : "false");
    if (in_alr && alr_limited_backoff_enabled_) {
      if (rate_control_.TimeToReduceFurther(at_time, prev_bitrate_)) {
		  DEBUG_EX_LOG("alr_limited_backoff_enabled_ is true, prev_bitrate:%lld, result.target_bitrate:%lld",
            prev_bitrate_.bps(),
		...
		DEBUG_EX_LOG("acked_bitrate:%lld, result.target_bitrate:%lld",
      ...
      // so that we (almost) always have a bitrate estimate.
		DEBUG_EX_LOG("reducing send rate by 50%% every 200 ms");
    ...
    if (probe_bitrate) {
		DEBUG_EX_LOG("probe bitrate: %lld", probe_bitrate.value().bps());
      result.probe = true;
      result.updated = true;
  ...
  if ((result.updated && prev_bitrate_ != result.target_bitrate) ||
	...
	DEBUG_EX_LOG("setting prev_bitrate to: %lld, result.updated:%s",
        prev_bitrate_.bps(),
...
void DelayBasedBwe::SetStartBitrate(DataRate start_bitrate) {
	DEBUG_EX_LOG("BWE setting start bitrate to: %s",
