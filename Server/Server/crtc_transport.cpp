...
namespace chen {
	...
	bool crtc_transport::_negotiate_publish_capability(crtc_source_description * stream_desc)
	{
		...
		for (int32 i = 0; i < (int)m_remote_sdp.m_media_descs.size(); ++i) 
		{
		...
		/*	SrsRtcTrackDescription* track_desc = new SrsRtcTrackDescription();
			...
			if (true) 
			{
				std::map<int, std::string> extmaps = remote_media_desc.get_extmaps();
				for (std::map<int, std::string>::iterator it = extmaps.begin(); it != extmaps.end(); ++it) 
				{
