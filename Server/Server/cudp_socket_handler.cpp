		...
		// bool fd_exp_reset = false;
		 ...
		 while (!m_closed)
		 {
			 ...
			 else
			 {
				 if (FD_ISSET(m_socket, &fd_read)) 
				 {
					...
					// int recv_byte = ::recvfrom(m_socket, (char *)m_ReadBuffer_ptr, ReadBufferSize, 0, );
					 ...
					 int32 bytes_read = ::recvfrom(m_socket, (char *)(m_ReadBuffer_ptr), ReadBufferSize, 0, (struct sockaddr*)&from, &from_len);
					 if (bytes_read > 0)
					 {
			 ...
			 while (temp_queue.size())
			 {
				 ...
				 sendto(m_socket, (char *)send_data_ptr->data, send_data_ptr->len, 0, &send_data_ptr->addr, sizeof(*&send_data_ptr->addr));
