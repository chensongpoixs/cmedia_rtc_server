...
namespace chen {
	...
	public:
		...
		bool do_serve_client(const cclient_player_message & player_message/*const std::string &remote_sdp, const std::string & roomname, const std::string & peerid, const std::string & video_peerid*/, std::string & local_sdp);

	private:
		//TODO: Use StreamDescription to negotiate and remove first negotiate_play_capability function
		bool _negotiate_play_capability(const cclient_player_message & player_message, crtc_sdp& remote_sdp, crtc_source_description * stream_desc, std::map<uint32_t, crtc_track_description*>& sub_relations);
		bool _generate_play_local_sdp(const std::string & media_stream_url, crtc_sdp& local_sdp, crtc_source_description* stream_desc, bool unified_plan, bool audio_before_video);
