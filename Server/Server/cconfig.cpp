...
namespace chen {
	...
	bool ccfg_file::init(const char * file_name)
	{
		...
		while (getline(input, line))
		{
			if (_parser_line(line, key, value))
			{
				...
				//m_configs[1].m_data = value; // ±£´æµ½mapÈÝÆ÷ÖÐµÄ·½·¨¡£
			}
		}
		if (input)
		{
	...
	bool ccfg_file::_parser_line(const std::string & line, std::string & key, std::string & value)
	{
		...
		std::string new_line = line.substr(start_pos, end_pos - start_pos + 1); // É¾µôºó°ë²¿·ÖµÄ×¢ÊÍ FIX_ME£º ÕâÀïÓ¦¸ÃÊÇ¼õ´íÁË°É
																				// ÏÂÃæposµÄ¸³ÖµÊ±±ØÒªµÄ£¬ÕâÑù£¬¾Í¿ÉÔÚºóÃæµÃµ½KeyºÍvalueÖµÁË¡£
		if ((pos = new_line.find("=")) == std::string::npos) //ËµÃ÷Ç°ÃæÃ»ÓÐ = ºÅ
		{
			return false;
		}
		key = new_line.substr(0, pos); // »ñµÃkey
		value = new_line.substr(pos + 1, end_pos + 1 - (pos + 1)); // »ñµÃvalue
		_trim(key);
		if (key.empty())
		{
			return false;
		}
		_trim(value); // ÒòÎªÕâÀïµÄkeyºÍvalue¶¼ÊÇÒýÓÃ´«µÝ£¬¿ÉÒÔÖ±½Ó±»ÐÞ¸Ä£¬ËùÒÔ²»ÓÃ·µ»Ø
		return true;
	}
	...
	void ccfg_file::_trim(std::string & str)
	{
		...
		start_pos = i; // »ñÈ¡µ½·Ç¿Õ¸ñµÄ³õÊ¼Î»ÖÃ
